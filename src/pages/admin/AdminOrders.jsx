  ...
  return (
    <div>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        ...
        {STATUSES.map((s) => (
          <button key={s} onClick={() => setFilter(s)} className={`rounded-sm px-4 py-2 text-xs uppercase ${filter === s ? 'bg-gold-400 text-ink-primary' : 'border border-ink-primary/20 text-ink-secondary'}`}>{s.replace(/_/g, ' ')}</button>
        ))}
      </div>
      ...
      <div className="mt-8 card-surface overflow-x-auto">
        <table className="w-full min-w-[800px] text-left text-sm">
          ...
          <tbody className="divide-y divide-surface-strong/20">
            {loading ? (
              Array.from({ length: 5 }).map((_, i) => <tr key={i}><td colSpan={6} className="p-3"><RowSkeleton /></td></tr>)
            ) : orders.length === 0 ? (
              <tr><td colSpan={6} className="px-5 py-10 text-center text-ink-inverse">No orders yet.</td></tr>
            ) : (
              orders.map((o) => (
                <tr key={o._id} className="text-ink-secondary">
                  ...
                  <td className="px-5 py-4">
                    ...
                    >
                      {STATUSES.map((s) => <option key={s} value={s} className="bg-surface-raised text-ink-primary">{s.replace(/_/g, ' ')}</option>)}
                    </select>
                  </td>
                  <td className="px-5 py-4 text-right">
                    <Link to={`/admin/orders/${o._id}`} className="text-xs text-gold-500 hover:underline">View</Link>
                  </td>
                </tr>
              ))
            )}
